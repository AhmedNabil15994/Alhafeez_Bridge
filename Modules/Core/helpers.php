...
if (!function_exists('getCategoryAttr')) {
    function getCategoryAttr($item)
    {
        $attrs = [];
        foreach ($item['custom_attributes'] as $attr) {
            ...
            if ($attr['value'] !== strip_tags($attr['value'])) {
                $doc = new \DOMDocument();
                $doc->loadHTML(htmlspecialchars_decode($value), LIBXML_NOERROR);
                $imageElements = $doc->getElementsByTagName('img');
                if (count($imageElements)) {
                    foreach ($imageElements as $oneImage) {
                        $image = $oneImage->getAttribute('src');
                        $image = str_replace('{{media url=.renditions/', '', $image);
                        $image = str_replace('{{media url=.', '', $image);
                        $image = str_replace('{{media url=', '', $image);
                        $image = str_replace('}}', '', $image);
                        if (!$image) {
                            $key = '';
                        }
                        $images[] = env('MAGENTO_MEDIA_URL') . $image;
                    }
